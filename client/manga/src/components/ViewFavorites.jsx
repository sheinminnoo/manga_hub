...
const ViewFavorites = () => {
    ...
    const handleRemoveFavorite = async (mangaId) => {
        const confirmRemove = window.confirm("Are you sure you want to remove this manga from your favorites?");
        if (confirmRemove) {
            try {
                await axios.delete(`/api/favorite/${mangaId}`);
                setFavorites(favorites.filter(fav => fav.manga && fav.manga._id !== mangaId));
